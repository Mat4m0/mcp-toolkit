...
5. For mixed-access servers, use a soft approach (set context, don't throw). For protected-only servers with OAuth endpoints, return 401 with WWW-Authenticate header
...
- **Soft approach (recommended for mixed-access servers):** Don't throw errors for missing auth. Set context when auth succeeds, allow requests to continue otherwise. Tools check context and return errors for unauthenticated users. This lets the same endpoint serve both public and authenticated tools.

- **Strict approach (for protected-only servers):** Return a `401` response with a `WWW-Authenticate: Bearer` header. This is the MCP-spec-compliant signal for clients that support OAuth discovery. Only use this when your server requires authentication for all operations and you have OAuth endpoints (`.well-known/oauth-*`) configured.

```ts
// Strict approach — only if you have OAuth endpoints set up
...
Most setups should use the soft approach shown below.
::
